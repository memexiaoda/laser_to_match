...
// #endif
                                       ...
                                       bool doPenalize, Pose2& rMean, Matrix3& rCovariance, bool doingFineMatch)
  {
    ...
    cout << "max response = " << bestResponse << endl;
    ...
    double sigma = 0.6;
    double ratio = 1/(sqrt(2 * 3.14) * sigma) * exp(-std::sqrt(l2_norm)  /(2 * sigma * sigma));
    std::cout << ratio << endl;
    bestResponse *= ratio;
  ...
  double ScanMatcher::GetResponse(unsigned int angleIndex, int gridPositionIndex) const
  {
    ...
    int cal_size;
    if(0 < response_set.size() < 8)
    {
      cal_size = response_set.size();
    }
    else if(response_set.size() >= 8)
    {
    ...
    std::cout<< "The cel is = " << cal_size << std::endl;
